...
// Ported verbatim from the Citation Atlas design (design/claude-design-project/force3d.js)
// with one production change: setData() consumes precomputed frozen positions
// (x/y/z from positions.bin, r from the loader) instead of running _layout() —
// the identical layout now runs offline in scripts/layout_clustered.mjs.
...
export class ForceGraph3D {
  ...
  setData(nodes, edges) {
    ...
    const hasPositions = this.allNodes.length && Number.isFinite(this.allNodes[0].x);
    if (hasPositions) {
      // frozen precomputed layout (positions.bin); just init velocities + radius
      for (const n of this.allNodes) {
        n.vx=n.vy=n.vz=0;
        if (!Number.isFinite(n.r)) n.r = 1.6 + Math.sqrt(n.citations)/22;
      }
    } else {
      this._layout();
    }
